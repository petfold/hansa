# hansa — credentials, cover and options: the cross-repository plan

Status: design, the plan this repository was created under (consolidated
2026-09-25). loopmarket and factbond carry the same text under their own
headers. hansa's side is D8, D9's hansa row, D10's product-layer
duties and the amendments D, E, F, G under them; the charter is
`../CHARTER.md`; the detailed designs are loopmarket's and factbond's. It resolves the
...
amendments from `../../../loopmarket/docs/plans/commercial-practice-review.md`. The round-by-round record
...
- **Per maker, in v1:** at most one active option record or unperformed
  ...
  deposit, point and fact cover as everything in loopmarket's `../../../loopmarket/docs/plans/items-and-ownership.md` §5.3.
...
- **Priced exclusivity, the first post-v6 addition**, its field shape
  fixed now (loopmarket's `../../../loopmarket/docs/plans/items-and-ownership.md` §8): a give naming `item(h)` may declare `exclusive:
...
**Consequences.** loopmarket's `../../../loopmarket/docs/plans/items-and-ownership.md` §2 and I2/I3 as rewritten; `exclusive` ships in
the bump after v6.
...
- **Names:** the record is `option/`; "hold" and "claim" keep their escrow
  meanings (§1); loopmarket's `../../../loopmarket/docs/plans/options-and-cover.md` says "one model, two routes".
...
**Consequences.** loopmarket's `../../../loopmarket/docs/plans/options-and-cover.md` §2, §3 as rewritten; the loop record's new field
...
- Door checks (D8) are witness types, routed through the roster as loopmarket's `../../../loopmarket/docs/plans/counterparty-gate.md`
  §7 says.

**Consequences.** loopmarket's `../../../loopmarket/docs/plans/counterparty-gate.md` as rewritten; `schema.py` once; `announce.py` reads
...
- Evidence classes are factbond's catalogue; the only ordered chains in the
  ...
  (ontodag-core's `../../../ontodag-core/docs/ASKS.md` and ontodag's `ROADMAP.md` §2): ontodag decides between the kind and cumulative
...
| **ontodag / ontodag-core** | the identifier kind (registry 4.3), the ordinal decision, the general vocabulary additions in ontodag-core's `../../../ontodag-core/docs/ASKS.md` and ontodag's `ROADMAP.md` §3, the legal-vocabulary check of §3b |
...
- **Good practice, not compliance.** loopmarket is open source on the
  ...
  against that record in `../../../loopmarket/docs/plans/commercial-practice-review.md` and amended
...
- loopmarket U1, U5: unchanged; options and cover are uniform offers with
  ...
  rejections enumerate. U11 extended as loopmarket's `../../../loopmarket/docs/plans/options-and-cover.md` §7 says. **U12 respected**:
...
| new plan doc, from loopmarket's `../../../loopmarket/docs/plans/options-and-cover.md` | options and cover (as rewritten, with D3, D4, D6) |
| new plan doc, from loopmarket's `../../../loopmarket/docs/plans/items-and-ownership.md` | items (as rewritten, with D5, E1, E2) |
| new plan doc, from loopmarket's `../../../loopmarket/docs/plans/counterparty-gate.md` | the gate (as rewritten, with D7, D8, E3, E4, G1, G2) |
...
| ontodag-core packs | `warranty`, `inspection`, `survey`, `certification`, `qualification`, `diploma`, `accreditation`, `theft`, `stolen-goods`, `recall`, `serial-number`, the batch sense of lot, `surety-bond` (§3 of the asks); the occupations pack unchanged; the legal-vocabulary check (ontodag-core's `../../../ontodag-core/docs/ASKS.md` and ontodag's `ROADMAP.md` §3b) |
...
(ontodag-core's `../../../ontodag-core/docs/ASKS.md` and ontodag's `ROADMAP.md` §3b) and the fold-in itself (§4).
